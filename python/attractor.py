...
def main() -> None:
    ...
    try:
        ...
        states1 = odeint(attractor.halvorsen, [x + np.random.uniform(-1.0, 1.0) for x in args.initialstate], steps)
        ...
        def _update(num):
            ...
            print(f"{(100*num/len(segments0)):.2f}% ({num} of {len(segments0)})", end="\r", flush=True)
            return lc0, lc1

        MAXTIME = 69                                #10 seconds
        NUMOFFRAMES = len(segments0)
        MAXINTERVAL = int(MAXTIME*1000/NUMOFFRAMES) #multiply by 1000 to convert to milliseconds
